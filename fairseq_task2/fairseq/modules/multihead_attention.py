...
class MultiheadAttention(FairseqIncrementalDecoder):
    ...
    def _xformers_attn_forward(
        ...
        q = self.q_proj(query,num_bits=8)
        k = self.k_proj(key,num_bits=8)
        v = self.v_proj(value,num_bits=8)
       
        q = self.quantize_input(q, num_bits=8)
        k = self.quantize_input(k, num_bits=8)
        v = self.quantize_input(v, num_bits=8)
    ...
    def forward(
        ...
        if self.self_attention:
            q = self.q_proj(query,num_bits=8)
            k = self.k_proj(query,num_bits=8)
            v = self.v_proj(query,num_bits=8)
        elif self.encoder_decoder_attention:
