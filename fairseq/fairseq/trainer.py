...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        self.cyclic_num_bits_schedule=[16,16]
        self._iter=0
        self.num_bits=-1
        self.num_cyclic_period=32
    ...
    def cyclic_adjust_precision(self, cyclic_period):
        ...
        assert len(self.cyclic_num_bits_schedule) == 2
        # assert len(self.cyclic_num_grad_bits_schedule) == 2

        num_bit_min = self.cyclic_num_bits_schedule[0]
        num_bit_max = self.cyclic_num_bits_schedule[1]

        # num_grad_bit_min = args.cyclic_num_grad_bits_schedule[0]
        # num_grad_bit_max = args.cyclic_num_grad_bits_schedule[1]
        # print("cyclic_period,",cyclic_period)
        self.num_bits = np.rint(num_bit_min +
                                0.5 * (num_bit_max - num_bit_min) *
                                (1 + np.cos(np.pi * ((self._iter % cyclic_period) / cyclic_period) + np.pi)))
        ...
        # if self._iter % self.num_cyclic_period == 0:
        #     logging.info('Iter [{}] num_bits = {} cyclic precision'.format(self._iter, self.num_bits))
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        cyclic_period = int(self.cfg.optimization.max_update / self.num_cyclic_period)
        ...
        with torch.no_grad():
            ...
            try:
                _loss, sample_size, logging_output = self.task.valid_step(
                    sample, self.model, self.criterion, num_bits=self.num_bits,**extra_kwargs
                )
            except RuntimeError as e:
                if "out of memory" in str(e):
                    self._log_oom(e)
                    if not raise_oom:
                        ...
                        if self.cuda:
                            torch.cuda.empty_cache()
                        return self.valid_step(sample, num_bits=self.num_bits,raise_oom=True)
                raise e
