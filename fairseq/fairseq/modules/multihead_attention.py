...
class MultiheadAttention(FairseqIncrementalDecoder):
    ...
    def _xformers_attn_forward(
        ...
        q = self.q_proj(query,num_bits=num_bits)
        k = self.k_proj(key,num_bits=num_bits)
        v = self.v_proj(value,num_bits=num_bits)
       
        q = self.quantize_input(q, num_bits=num_bits)
        k = self.quantize_input(k, num_bits=num_bits)
        v = self.quantize_input(v, num_bits=num_bits)
        ...
        y = self.out_proj(y)
        y=self.quantize_input(y, num_bits=num_bits)
    ...
    def forward(
        ...
        if self.self_attention:
            q = self.q_proj(query,num_bits=num_bits)
            k = self.k_proj(query,num_bits=num_bits)
            v = self.v_proj(query,num_bits=num_bits)
        elif self.encoder_decoder_attention:
            ...
            if key is None:
                ...
                k = self.k_proj(key,num_bits=num_bits)
                v = self.v_proj(key,num_bits=num_bits)
