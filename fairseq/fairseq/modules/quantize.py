    ...
    def forward(self, input, num_bits, qparams=None):
        ...
        if self.training or self.measure:
            ...
            with torch.no_grad():
                ...
                # print("PRINTING HER",momentum,self.running_zero_point,qparams.zero_point)
...
class QLinear(nn.Linear):
    def __init__(self, in_channels, out_channels, bias=True):
        super(QLinear, self).__init__(in_channels, out_channels, bias=True)
        self.linear = nn.Linear(in_channels, out_channels, bias)
        self.quantize_input = QuantMeasure(shape_measure=(1,1), flatten_dims=(1,-1))


    def forward(self, input, num_bits=None, num_grad_bits=None):
        qinput = self.quantize_input(input, 16)
        
        # Adjust here to access self.linear.weight
        weight_qparams = calculate_qparams(self.linear.weight, num_bits=num_bits, flatten_dims=(1, -1), reduce_dim=None)
        qweight = quantize(self.linear.weight, qparams=weight_qparams)
        # print("quantinize called")
        output = F.linear(qinput, qweight, self.linear.bias)  # Include bias in the linear operation if needed
        return output

    # Optionally, include a method to quantize weights based on the pre-calculated qparams
    def quantize_weights(self, num_bits):
        qparams = calculate_qparams(self.linear.weight, num_bits=num_bits, reduce_dim=None)
        self.linear.weight.data = quantize(self.linear.weight, qparams=qparams)
        if self.linear.bias is not None:
            self.linear.bias.data = quantize(self.linear.bias, qparams=qparams)





# CUDA_VISIBLE_DEVICES=0 fairseq-hydra-train --config-dir examples/roberta/config/finetuning --config-name rte 'task.data="/mnt/c/Users/yifei/OneDrive/桌面/CS8803EML/eml-hw2/fairseq/RTE-bin"' 'checkpoint.restore_file="/mnt/c/Users/yifei/OneDrive/桌面/CS8803EML/eml-hw2/roberta.base/model.pt"' > rte_10ep_unmodified.txt