...
class FairseqTask(object):
    ...
    def train_step(
        self, sample, model, criterion, optimizer, update_num, num_bits, ignore_grad=False
    ):
        """
        ...
        with torch.autograd.profiler.record_function("forward"):
            with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                loss, sample_size, logging_output = criterion(model, sample, num_bits)
        if ignore_grad:
            loss *= 0
    ...
    def valid_step(self, sample, model, criterion,num_bits):
        model.eval()
        with torch.no_grad():
            loss, sample_size, logging_output = criterion(model, sample,num_bits)
        return loss, sample_size, logging_output
